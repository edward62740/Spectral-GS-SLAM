...
public:
  ...
  // --- New Methods for Inflation ---
  void apply_source_covariance_weights(const std::vector<float>& weights);
  void restore_source_covariances();
  
  std::pair<std::vector<double>, std::vector<double>> get_residuals() const;
  // --------------------------------

  // --- New Methods for Weighting ---
  void setTargetWeights(const std::vector<float>& weights);
  // --------------------------------
...
protected:
  ...
  std::vector<Eigen::Matrix4d, Eigen::aligned_allocator<Eigen::Matrix4d>> source_covs_clean_; // Clean backup
