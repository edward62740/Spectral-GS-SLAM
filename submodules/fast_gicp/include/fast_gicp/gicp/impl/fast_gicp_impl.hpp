...
void FastGICP<PointSource, PointTarget, SearchMethodSource, SearchMethodTarget>::calculateSourceCovariance() {
	...
	// Save CLEAN covariances for later inflation
	source_covs_clean_ = source_covs_;
}
...
void FastGICP<PointSource, PointTarget, SearchMethodSource, SearchMethodTarget>::computeTransformation(PointCloudSource& output, const Matrix4& guess) {
  ...
  // Initialize SPRT containers
  this->sprt_ratios_.assign(input_->size(), 0.0);
  this->initial_residuals_.assign(input_->size(), -1.0); // -1.0 indicates not set
  
  LsqRegistration<PointSource, PointTarget>::computeTransformation(output, guess);
  LsqRegistration<PointSource, PointTarget>::computeTransformation(output, guess);
  
  // Update residuals and weights for the final transformation
  this->linearize(Eigen::Isometry3d(this->final_transformation_.template cast<double>()), nullptr, nullptr);
}

// --- New Methods Implementation ---
template <typename PointSource, typename PointTarget, typename SearchMethodSource, typename SearchMethodTarget>
void FastGICP<PointSource, PointTarget, SearchMethodSource, SearchMethodTarget>::restore_source_covariances() {
    if (source_covs_clean_.size() != source_covs_.size()) {
        // Only if we haven't initialized clean yet, or sizes mismatch (shouldn't happen if managed correctly)
        return; 
    }
    source_covs_ = source_covs_clean_;
}

template <typename PointSource, typename PointTarget, typename SearchMethodSource, typename SearchMethodTarget>
void FastGICP<PointSource, PointTarget, SearchMethodSource, SearchMethodTarget>::apply_source_covariance_weights(const std::vector<float>& weights) {
    if (source_covs_.size() != weights.size()) {
        std::cerr << "[FastGICP] Weight size mismatch! Covs: " << source_covs_.size() << " Weights: " << weights.size() << std::endl;
        return;
    }
    
    // Ensure clean covs exist
    if (source_covs_clean_.empty()) {
        source_covs_clean_ = source_covs_;
    }

    float max_inflation = 20.0f;
    float epsilon = 1e-6f;

    #pragma omp parallel for num_threads(num_threads_)
    for (int i = 0; i < source_covs_.size(); i++) {
        float w = weights[i];
        // 1. Compute Inflation Scale (Same mechanism as Python)
        // w is "Anchor Probability" (0.0 to 1.0)
        // Scale = 1.0 / (0.05 + 0.95*w) -> Max 20.0
        float inflation_scale = 1.0f / (0.05f + 0.95f * w);

        // 2. Apply Inflation to CLEAN covariance
        // We modify source_covs_[i] in place
        // Note: We only scale the 3x3 diagonal block to preserve rotation structure?
        // Actually, scaling the whole 3x3 matrix by scalar is equivalent to scaling eigenvalues.
        
        source_covs_[i].block<3, 3>(0, 0) = source_covs_clean_[i].block<3, 3>(0, 0) * inflation_scale;
        
        // 3. Regularize (Add Epsilon)
        source_covs_[i](0, 0) += epsilon;
        source_covs_[i](1, 1) += epsilon;
        source_covs_[i](2, 2) += epsilon;
        
        // 4. Handle 4th Dimension (Homogeneous)
        // Ensure it's safe for inversion
        source_covs_[i](3, 1) = 0.0;
        source_covs_[i](3, 2) = 0.0;
    }
}

template <typename PointSource, typename PointTarget, typename SearchMethodSource, typename SearchMethodTarget>
void FastGICP<PointSource, PointTarget, SearchMethodSource, SearchMethodTarget>::setTargetWeights(const std::vector<float>& weights) {
    target_weights_ = weights;
}

template <typename PointSource, typename PointTarget, typename SearchMethodSource, typename SearchMethodTarget>
std::pair<std::vector<double>, std::vector<double>> FastGICP<PointSource, PointTarget, SearchMethodSource, SearchMethodTarget>::get_residuals() const {
  if (last_residuals_.size() != input_->size()) {
      // Fallback or empty if not computed yet
      return {std::vector<double>(input_->size(), 0.0), std::vector<double>(input_->size(), 0.0)};
  }
  return {last_residuals_, last_weights_};
}
...
double FastGICP<PointSource, PointTarget, SearchMethodSource, SearchMethodTarget>::linearize(const Eigen::Isometry3d& trans, Eigen::Matrix<double, 6, 6>* H, Eigen::Matrix<double, 6, 1>* b) {
  ...
  // Resize buffers
  if(last_residuals_.size() != input_->size()) {
      last_residuals_.resize(input_->size());
      last_weights_.resize(input_->size());
  }
...
#pragma omp parallel for num_threads(num_threads_) reduction(+ : sum_errors) schedule(guided, 8)
  for (int i = 0; i < input_->size(); i++) {
    ...
    double mahalanobis_sq = error.transpose() * mahalanobis_[i] * error;
    sum_errors += mahalanobis_sq;
    
    last_residuals_[i] = mahalanobis_sq;

    
    if (this->initial_residuals_.size() == input_->size()) {
        if (this->initial_residuals_[i] < 0.0) {
            this->initial_residuals_[i] = mahalanobis_sq;
        }
        
        double final_sq = mahalanobis_sq;
        double initial_sq = this->initial_residuals_[i];
        

        this->sprt_ratios_[i] = std::min(initial_sq, final_sq);
    }

    // huber
    double w = 1.0;
    if(this->use_robust_loss_) {
       double error_val = std::sqrt(mahalanobis_sq);
       if(error_val > this->robust_k_) {
         w = this->robust_k_ / error_val;
       }
    }


    if (this->use_inverse_depth_weighting_) {
        double depth = mean_A.z();
        double w_depth = 1.0 / (std::max(0.1, depth)); 
        w *= (w_depth * w_depth);
    }

    // reliability
    if (!target_weights_.empty()) {
        if (target_index < target_weights_.size()) {
             w *= target_weights_[target_index];
        }
    }
    ...
    Eigen::Matrix3d R = trans.linear();
    dtdx0.block<3, 3>(0, 0) = R * skewd(mean_A.head<3>());
    dtdx0.block<3, 3>(0, 3) = -R;
